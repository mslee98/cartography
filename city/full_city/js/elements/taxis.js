var taxis = function( exports ){
    ...
    exports.init = function( group, camera ){
        ...
        taxiCamera = camera.clone();//new THREE.PerspectiveCamera( 40, camera.aspect, .1, 10000000 );
    ...
    exports.updateTaxiCam = function(t){
        ...
        taxiCamera.position.x += ( pos.x - taxiCamera.position.x  ) * .1;
        taxiCamera.position.y += ( 100 - taxiCamera.position.y  ) * .1;
        taxiCamera.position.z += ( pos.y - taxiCamera.position.z  ) * .1;
        var xy = getPositionAt(vertices, t+0.00001);
        tg.x += ( xy.x  - tg.x ) * .1;
        tg.y += ( 5     - tg.y ) * .1;
        tg.z += ( xy.y  - tg.z ) * .1;
        taxiCamera.lookAt( tg );
    };
